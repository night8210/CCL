...
def LR(data,iterations):
    train_data, test_data = data.randomSplit([0.7, 0.3], 10)
...
def DT(data, categoricalFeaturesInfo):
    train_data, test_data = data.randomSplit([0.7, 0.3], 10)
    train_data_collected = train_data.collect()
    _Classes = set()
    for i in range(len(train_data_collected)):
    ...
    dt_model = DecisionTree.trainClassifier(train_data, _numClasses, categoricalFeaturesInfo)
...
def RF(data, categoricalFeaturesInfo, numTrees):
    train_data, test_data = data.randomSplit([0.7, 0.3], 10)
    train_data_collected = train_data.collect()
    _Classes = set()
    for i in range(len(train_data_collected)):
    ...
    rf_model = RandomForest.trainClassifier(train_data, _numClasses, categoricalFeaturesInfo, numTrees)
...
def NB(data):
    ...
    nb_model = NaiveBayes.train(train_data)
